...
@Fork(value = 2, jvmArgsAppend = "-XX:StartFlightRecording=dumponexit=true,filename=/tmp/AimdLimitAlgorithmTimeoutBenchmark.jfr")
@Warmup(iterations = 5, time = 3)
@Measurement(iterations = 5, time = 3)
public class AimdLimitAlgorithmTimeoutBenchmark {
    ...
    @Benchmark
    public void benchmarkTimeoutRequest(Blackhole blackhole) {
    ...
    public void setupIteration() {
        ...
        rtt = ThreadLocalRandom.current().nextLong(Duration.ofMillis(1).toNanos(), Duration.ofMinutes(1).toNanos());
